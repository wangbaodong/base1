...
//	I2C_EE_BufferWrite(writeBuf,0x00,32);
  ...
  /* USER CODE END RTOS_EVENTS */
	osTimerStart(myTimer01Handle,1*10*1000);
...
{		 
  ...
  {	
		...
		uxBits = osEventFlagsWait(iwdgEventHandle,TASK_BIT_ALL,0x03,3000);
...
void PeriodicLogCallback(void *argument){
				...
				res_sd = FATFS_LinkDriver(&SD_Driver, SDPath);
				...
				{
					/* 移动指针到文件末尾 */
					f_lseek(&fnew,f_size(&fnew) - 1);//move the pointer to last word of the file
