...
/* ----------------------- Defines  -----------------------------------------*/
#define MB_TCP_DEFAULT_PORT 	502     			//默认监听端口
#define MB_TCP_BUF_SIZE     ( 256 + 7 ) 		//TCP控制块缓冲大小，必须大于等于一个完整大Modbus帧
...
#endif
                ( void )xMBTCPPortEventPost( EV_FRAME_RECEIVED );
            }
            else
            {
#ifdef MB_TCP_DEBUG
